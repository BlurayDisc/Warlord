...
public abstract class Player extends Unit {
	...
	public void acquire(Item item) {
		if (item instanceof RandomizableItem) {
			this.randomize((RandomizableItem) item);
		}
		inventory.put(item);
	}
	
	/**
	 * When the player acquires an item from smithing or crafting, the item's attributes 
	 * are calculated based on the level of the player's smithing or crafting skills, 
	 * with random factor taking place.
	 * @param item
	 */
	private void randomize(RandomizableItem item) {
